...
void initdat(std::vector< std::vector< double > > &arr, int namx);
...
int main(int argc, char* argv[])
{
    if (argc == 6)
    {
        ...
        bool PLOTFLAG = atoi(argv[4]);
        int MAXTHREADS = atoi(argv[5]);

        omp_set_num_threads(MAXTHREADS);

        if (LebwohlLasher(PROGNAME, ITERATIONS, SIZE, TEMPERATURE, PLOTFLAG))
            return EXIT_SUCCESS;
        else
            return EXIT_FAILURE;
    }
    ...
    {
        std::cout << "Usage: " << argv[0] << " <ITERATIONS> <SIZE> <TEMPERATURE> <PLOTFLAG> <THREADCOUNT>" << std::endl;
    }
}
