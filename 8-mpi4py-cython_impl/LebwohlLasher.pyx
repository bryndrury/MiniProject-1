...
@cython.boundscheck(False)
def all_energy(double[:, :]arr, int nmax, int rank, int size, int numthreads):
...
@cython.boundscheck(False)
def get_order(double[:, :] arr, int nmax, int rank, int size, int numthreads):
    omp_set_num_threads(numthreads)
...
@cython.boundscheck(False)
def MC_step(double[:, :] arr, double Ts, int nmax, int rank, int size, int numthreads):
    omp_set_num_threads(numthreads)
    
    # MPI variables
    cdef int start = rank * nmax // size
    cdef int end = (rank + 1) * nmax // size
    if rank == size - 1:
        end = nmax
    ...
    # Convert arr to a NumPy array before using np.copyto
    arr_np_full = np.asarray(arr)
    np.copyto(arr_np_full, new_arr)
    
    reduced_accept = float(accept)
    reduced_accept = MPI.COMM_WORLD.reduce(reduced_accept, op=MPI.SUM, root=0)
